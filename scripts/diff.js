...
function alignTagRuns(a, b, c, d, f) {
    ...
    var g = [];
    eachOpcode(a, function(a, n, m, p, k) {
        if ("replace" != a[0]) g.push(a);
        else {
            var e = hasTags(d.slice(n, m));
            a = hasTags(f.slice(p, k));
            if (e || a) {
                var l = n,
                    q = m;
                if (e)
                    for (e = n; e < m && !b[e].match(/^</); e++) l++;
                for (e = m - 1; e >= n &&
                    !b[e].match(/^</); e--) q--;
                var s = p,
                    t = k;
                if (a)
                    for (e = p; e < k && !c[e].match(/^</); e++) s++;
                for (e = k - 1; e >= p && !c[e].match(/^</); e--) t--;
                h(n, l, p, s, !0);
                h(l, q, s, t);
                h(q, m, t, k, !0)
            } else h(n, m, p, k, !0)
...
function recurseHtmlDiff(a, b, c, d, f, h) {
    ...
    eachOpcode(a, function(a, n, m, p, k) {
        if ("replace" != a[0]) g.push(a);
            ...
            if (e != l) {
                var e = Math.min(e, l),
                    l = getSimilarity(b[n], c[p]),
                    q = getSimilarity(b[m - 1], c[k - 1]);
                l > q ? (l = n + e, e = p + e, m != l ? a = ["delete", l, m, 0, 0] : k != e && (a = ["insert", 0, 0, e, k]), m = l) : (l = m - e, k -= e, n != l ? g.push(["delete", n, l, 0, 0]) : p != k && g.push(["insert", 0, 0, p, k]), n = l, p = k)
            }
            for (k = 0; k < m - n; k++) {
                var e = n + k,
                    l = p + k,
                    q = b[e].match(/^<(\w+)[^>]*>/),
                    s = c[l].match(/^<(\w+)[^>]*>/);
                q && s && q[0] == s[0] ? (q = [s[0], calculateHtmlDiff($(b[e]).html(), $(c[l]).html(), h), "</" + s[1] + ">"].join(""), s = hashToken(q, h), b[e] = c[l] = q, d[e] = f[l] = s, g.push(["equal", e, e + 1, l, l + 1])) : (g.push(["delete", e, e + 1, 0, 0]), g.push(["insert", 0, 0, l, l + 1]))
            }
            a && g.push(a)
        }
    });
...
function clusterRuns(a, b, c, d, f) {
    var h = [],
        g = [],
        r = [];
    eachOpcode(a, function(a, m, p, k, e) {
        var l = hasTags(d.slice(m, p)),
            q = hasTags(f.slice(k, e));
        switch (a[0]) {
            case "delete":
                g.push(a);
                break;
            case "insert":
                ...
                break;
            case "equal":
                if ((g.length || r.length) && !l && !q && b.slice(m, p).join("").length < SHORT_TEXT_LENGTH && c.slice(k, e).join("").length < SHORT_TEXT_LENGTH) {
                    g.push(["delete", m, p, k, e]);
                    r.push(["insert", m, p, k, e]);
                    break
                }
            case "replace":
                h = h.concat(g), h = h.concat(r), g = [], r = [], h.push(a)
        }
    });
    h = h.concat(g);
    return h = h.concat(r)
...
function assembleHtmlDiff(a, b, c) {
    var d = [],
        f = "equal";
    eachOpcode(a, function(a, g, r, n, m) {
        "equal" != f && f != a[0] && d.push("delete" == f ? DEL_END : INS_END);
        switch (a[0]) {
            case "delete":
                g != r && ("delete" != f && d.push(DEL_START), d = d.concat(b.slice(g, r)));
                break;
            case "insert":
                n != m && ("insert" != f && d.push(INS_START), d = d.concat(c.slice(n, m)));
                break;
            case "equal":
                d = d.concat(c.slice(n, m));
                break;
            default:
...
function generateControls(a) {
    var b = !getSetting(SETTINGS.hide_deletions),
        ...
        r = chrome.i18n.getMessage("diff_show_deletions");
    a = '<div id="chrome_page_monitor_ext_orig_link">                     <a class="pm_original" href="%url%"                        title="%title%">%original%</a>                     <br />                     <a class="pm_textize" href="#">%textize%</a>                     <br />                     <a class="pm_hide" href="#">%hide%</a>                   </div>'.replace("%url%", a).replace("%title%",
        c).replace("%original%", d).replace("%textize%", f).replace("%hide%", b ? g : r);
    a = $(a);
    b || $("del").hide();
    ...
    var n = $('link[rel="stylesheet"]:not([href="styles/diff.css"]),style'),
        m = $('<link rel="stylesheet" type="text/css" href="diff_txt.css"/>'),
        p = $("body *[style]").each(function() {
    ...
    $(".pm_textize", a).click(function() {
        $(this).text($(this).text() ==
            h ? f : h);
        e ? (n.appendTo("head"), m.detach(), p.each(function() {
            $(this).attr("style", $(this).data("style"))
        }), k.show()) : (n.detach(), m.appendTo("head"), p.each(function() {
            $(this).attr("style", "")
        }), k.hide());
        e = !e;
        return !1
