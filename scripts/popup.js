...
function monitorCurrentPage() {
    ...
    chrome.tabs.getSelected(null, function(a) {
        addPage({
            ...
            if (b)
                for (var d = 0; d < b.length; d++) b[d].fillPagesList && b[d].fillPagesList()
        })
    })
}
    ...
    getAllUpdatedPages(function(b) {
        ...
        0 < b.length ? $.each(b, function(a, b) {
            var c = $("#templates .notification").clone(),
                e = trimPageName(b);
            c.find(".page_link").attr("href", b.url).text(e);
            c.find(".favicon").attr({
...
function updateButtonsState() {
    chrome.tabs.getSelected(null, function(a) {
        isPageMonitored(a.url, function(b) {
            b || !a.url.match(/^https?:/) ? ($("#monitor_page").unbind("click").addClass("inactive"), $("#monitor_page img").attr("src", "img/monitor_inactive.png"), b = b ? "page_monitored" : "monitor", $("#monitor_page span").text(chrome.i18n.getMessage(b))) : ($("#monitor_page").click(monitorCurrentPage).removeClass("inactive"), $("#monitor_page img").attr("src", "img/monitor.png"), $("#monitor_page span").text(chrome.i18n.getMessage("monitor")))
        })
    });
    ...
    getAllPageURLs(function(a) {
        getAllUpdatedPages(function(b) {
            a.length == b.length ? ($("#check_now").addClass("inactive"), $("#check_now img").attr("src", "img/refresh_inactive.png")) : ($("#check_now").removeClass("inactive"), $("#check_now img").attr("src", "img/refresh.png"))
        })
    })
}
...
function openAllPages() {
    var a = "diff" == getSetting(SETTINGS.view_all_action) ? "view_diff" : "page_link";
